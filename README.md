...
1. 관리자가 전날 `data/algorithm-schedule.json`에 다음 날 문제 3개를 등록합니다.
2. 매일 오전 7시(KST)에 상·중·하 Issue가 각각 자동 생성됩니다.
3. Discord `#algorithm_study` 채널에 문제별 커스텀 알림 3개가 전송됩니다.
4. 학생은 자신의 Fork를 최신 상태로 동기화하고 풀이 브랜치를 만듭니다.
5. 풀이를 Commit·Push하고 공용 저장소의 `main` 브랜치로 Pull Request를 만듭니다.
6. PR 생성·수정·리뷰·병합 상태는 Discord의 GitHub 웹훅으로 공유됩니다.
...
SWEA의 문제 목록과 접근 방식이 변경될 수 있어 무작위 스크래핑으로 문제를 고르지 않습니다. 관리자가 직접 확인한 문제 링크를 등록하는 방식이 가장 안정적입니다.

## 관리자가 문제를 등록하는 방법

`data/algorithm-schedule.json`의 `_example`을 복사하고 날짜를 `YYYY-MM-DD`로 변경합니다.
...
실제 저장 시에는 설명용 날짜와 가짜 링크를 사용하지 말고 SWEA에서 복사한 실제 문제 링크를 입력합니다. PR의 `알고리즘 자동화 검증` 체크가 날짜, 필수 필드, 난이도 구간, URL을 검사합니다.

오늘 날짜의 문제가 없으면 자동화는 임의의 문제를 만들지 않고 실패합니다. 이 실패 알림을 받은 관리자는 일정을 추가한 뒤 Actions를 수동 실행하면 됩니다.
